...
import (
  ...
  // Try rez instead? (and put this into internal folder...)
...
// Save several image representation rescaled in proportion (always in proportion)
// using the specified max width, max height and quality
// only one of max width or height is used depending on the proportions
func SaveJpegRepresentations(r io.Reader, options []Options) error {
    ...
    for _,o := range options {
       ...
       defer w.Close()
       err = jpeg.Encode(w, resized, &jpeg.Options{o.Quality})
       if err != nil {
           return err
       }
...
// Resize an image - NB the image is untouched if it is not larger than maxWidth or maxHeight
